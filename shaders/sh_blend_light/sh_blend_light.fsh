...
#define eLightType_direction_facingectional 3
...
uniform float u_Lightdirection_facingection;
...
// Gets the angle difference from the coord to the light's direction_facingection
float angleDiff(vec2 origin, vec2 coord, float direction_facingection);
...
void main()
{
	...
	/*								*/
		...
		intensity *= clamp(u_LightAngle * 0.5 / angleDiff(u_LightPosition, v_vTexcoord, u_Lightdirection_facingection), 0.0, 1.0);
	}
	/*								*/
	/*		 Area & Line Light		*/
	/*								*/
...
float angleDiff(vec2 origin, vec2 coord, float direction_facingection) {
	...
	return abs(mod(mod(angle - direction_facingection, 360.0) + 540.0, 360.0) - 180.0);
}
